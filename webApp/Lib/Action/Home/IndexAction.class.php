...
class IndexAction extends Action {
	...
	public function tengfei() {
		...
		*/
		...
		$navlist = array ('0' => array ("url" => '__URL__/tengfei/type/rhtf', "title" => '如何腾飞' ), '1' => array ("url" => '__URL__/tengfei/type/tfzw', "title" => '腾飞展望' ) );
		...
		if ($type == "rhtf" || $type == "tfzw") //如何腾飞和腾飞展望
{
			...
			foreach ( $temp as $one => $value ) {
				$maincontent [$num ++] = array ('title' => $value [incubatorname], 'content' => $value [incubatorcontent] );
			}
			...
			foreach ( $temp as $one => $value ) {
				$maincontent [$num ++] = array ('title' => $value [modulename], 'content' => $value [modulecontent] );
			}
		} else //首页进来为该孵化器平台的简介
{
			//数据库
